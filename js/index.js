const dom = {
    ...
    namedPlaylistSelect: document.getElementById("namedPlaylistSelect"),
...
const savedActiveNamedPlaylistId = (() => {
    const stored = safeGetLocalStorage("activeNamedPlaylistId");
    return savedNamedPlaylists.some((playlist) => playlist.id === stored)
        ? stored
        : savedNamedPlaylists[0].id;
})();

const initialNamedPlaylist = savedNamedPlaylists.find((playlist) => playlist.id === savedActiveNamedPlaylistId)
    || savedNamedPlaylists[0];
...
const state = {
    ...
    playlistSongs: initialNamedPlaylist.songs.slice(),
    namedPlaylists: savedNamedPlaylists,
    activeNamedPlaylistId: savedActiveNamedPlaylistId,
...
function getActiveNamedPlaylist() {
    return state.namedPlaylists.find((playlist) => playlist.id === state.activeNamedPlaylistId)
        || state.namedPlaylists[0]
        || null;
}

function syncActiveNamedPlaylistFromQueue() {
    const playlist = getActiveNamedPlaylist();
    if (playlist) {
        playlist.songs = Array.isArray(state.playlistSongs) ? state.playlistSongs.slice() : [];
    }
}

function saveNamedPlaylists(options = {}) {
    const { skipRemote = false } = options;
    syncActiveNamedPlaylistFromQueue();
    safeSetLocalStorage("namedPlaylists", JSON.stringify(state.namedPlaylists), { skipRemote });
    safeSetLocalStorage("activeNamedPlaylistId", state.activeNamedPlaylistId, { skipRemote });
}
...
function resetPlaybackForPlaylistSwitch() {
...
function switchNamedPlaylist(id) {
    if (!id || id === state.activeNamedPlaylistId) return;
    saveNamedPlaylists();
    const next = state.namedPlaylists.find((playlist) => playlist.id === id);
    if (!next) return;
    state.activeNamedPlaylistId = next.id;
    state.playlistSongs = next.songs.slice();
    resetPlaybackForPlaylistSwitch();
    renderNamedPlaylistSelector();
    renderPlaylist();
    saveNamedPlaylists();
    showNotification(`已切换到「${next.name}」`, "success");
}

function createNamedPlaylist() {
    const name = window.prompt("新歌单名称", `我的歌单 ${state.namedPlaylists.length + 1}`);
    if (!name || !name.trim()) return;
    saveNamedPlaylists();
    const id = `playlist-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`;
    state.namedPlaylists.push({ id, name: name.trim().slice(0, 40), songs: [] });
    state.activeNamedPlaylistId = id;
    state.playlistSongs = [];
    resetPlaybackForPlaylistSwitch();
    renderNamedPlaylistSelector();
    renderPlaylist();
    saveNamedPlaylists();
    showNotification("新歌单已建立", "success");
}
...
function deleteNamedPlaylist() {
    if (state.namedPlaylists.length <= 1) {
...
async function bootstrapPersistentStorage() {
    try {
        ...
        remoteSyncEnabled = await persistentStorage.checkAvailability();
    }
}
...
function applyPersistentSnapshotFromRemote(data) {
    ...
    let restoredNamedPlaylists = false;
        ...
        if (Array.isArray(playlists) && playlists.length > 0) {
            ...
            if (state.namedPlaylists.length > 0) {
                const requestedId = typeof data.activeNamedPlaylistId === "string"
                    ...
                    : state.namedPlaylists[0].id;
                state.activeNamedPlaylistId = state.namedPlaylists.some((playlist) => playlist.id === requestedId)
                    ? requestedId
                    : state.namedPlaylists[0].id;
                const active = getActiveNamedPlaylist();
                state.playlistSongs = active ? active.songs.slice() : [];
                safeSetLocalStorage("namedPlaylists", JSON.stringify(state.namedPlaylists), { skipRemote: true });
                safeSetLocalStorage("activeNamedPlaylistId", state.activeNamedPlaylistId, { skipRemote: true });
                playlistUpdated = true;
                restoredNamedPlaylists = true;
                renderNamedPlaylistSelector();
            }
        }
    }

    if (!restoredNamedPlaylists && typeof data.playlistSongs === "string") {
...
function importSelectedSearchResults(target = "playlist", targetPlaylistId = state.activeNamedPlaylistId) {
    ...
    const songsToAdd = indices
        ...
        .filter((song) => song && typeof song === "object" && song.playable !== false);
...
const EXPLORE_RADAR_SOURCES = ["joox", "bilibili"];
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (dom.renamePlaylistBtn) {
        dom.renamePlaylistBtn.addEventListener("click", renameNamedPlaylist);
    }
    if (dom.deletePlaylistBtn) {
